...
SCENARIO("Constructing a camera")
{
  GIVEN("")
  {
    unsigned hsize = 160;
    unsigned vsize = 120;
    double fieldOfView = std::numbers::pi / 2;
    WHEN("Creating camera")
    {
      ...
      THEN("")
      {
        REQUIRE(c.hsize() == 160);
        REQUIRE(c.vsize() == 120);
        REQUIRE(c.fieldOfView() == std::numbers::pi / 2);
...
SCENARIO("The pixel size for a horizontal canvas")
{
  GIVEN("")
  {
    auto c = rt::Camera(200, 125, std::numbers::pi/2);
    THEN("")
    {
...
SCENARIO("Constructing a ray through the center of the canvas")
{
  GIVEN("")
  {
    auto c = rt::Camera(201, 101, std::numbers::pi/2);
    WHEN("")
    {
...
SCENARIO("Constructing a ray when camera is transformed")
{
  ...
  {
    auto c = rt::Camera(201, 101, std::numbers::pi/2);
    c.setTransform(util::Matrixd::rotation_y(std::numbers::pi/4) * util::Matrixd::translation(0,-2,5));
    WHEN("")
    {
      auto r = rt::rayForPixel(c,100,50);
      THEN("")
...
SCENARIO("Rendering a world with a camera") {
  GIVEN("") {
    ...
    auto c = rt::Camera(11,11,std::numbers::pi/2);
